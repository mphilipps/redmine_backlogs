...
module Backlogs
  class Statistics
    def initialize(project)
      ...
      @past_sprints = RbSprint.
        where("project_id = ? and not(effective_date is null or sprint_start_date is null) and effective_date < ?", @project.id, Date.today).
  ...
  module ProjectPatch
    def self.included(base) # :nodoc:
      ...
      base.class_eval do
        unloadable
        has_many :releases, -> {
          order("#{RbRelease.table_name}.release_start_date DESC, #{RbRelease.table_name}.name DESC")
        }, :class_name => 'RbRelease', :inverse_of => :project, :dependent => :destroy
      ...
      def open_shared_sprints
        if Backlogs.setting[:sharing_enabled]
          ...
          shared_versions.
            ...
            order("sprint_start_date #{order}, effective_date #{order}").
